...
async function getSession(
  ...
  try {
    let session = await Session.findOne({ sessionId, userId }).select(
      "-_id -createdAt -updatedAt -success -userId -allysonCost -pendingInterruption -podStatus -globalDetails -globalVariables"
    );
    if (!session) {
    ...
    if (
      ...
      (await isUrlExpired(session.lastScreenshotUrl))
    ) {
      ...
      const newScreenshotUrl = await getSignedUrl(s3Client, command, {
        expiresIn: 604800,
      }); // 7 days
    ...
    if (session) {
      // Remove the costs object from each message
      if (session.messages) {
        session.messages = session.messages.map((message) => {
          const { costs, agent_data, ...rest } = message;

          if (agent_data) {
            const { action } = agent_data;

            if (action) {
              return { ...rest, agent_data: { action } };
            }
          }
          return rest;
        });
